...
cp /storage/brno3-cerit/home/vlkofly/mockingbird_genomics/fastq_vcf_workshop/mapping/../fastq_data/Arenosa5_R1.fastq.gz $SCRATCHDIR
cp /storage/brno3-cerit/home/vlkofly/mockingbird_genomics/fastq_vcf_workshop/mapping/../fastq_data/Arenosa5_R2.fastq.gz $SCRATCHDIR
...
bwa mem -M -R '@RG\tID:'"$lanename"'\tLB:'"$lanename"'one\tPL:illumina\tSM:TBG_06ta' -t 3 JIC_reference/alygenomes.fasta outtrim/trim_*Arenosa5_R1*_1P.fastq.gz outtrim/trim_*Arenosa5_R1*_2P.fastq.gz | samtools view -@ 3 -bu - | samtools sort - -@ 3 -m 5G -o sort_Arenosa5_R1_paired.bam || exit 1  
echo paired reads mapped at `date`

bwa mem -M -R '@RG\tID:'"$lanename"'\tLB:'"$lanename"'one\tPL:illumina\tSM:TBG_06ta' -t 3 JIC_reference/alygenomes.fasta outtrim/trim_*Arenosa5_R1*_1U.fastq.gz | samtools view -@ 3 -bu - | samtools sort - -@ 3 -m 5G -o sort_Arenosa5_R1_unpaired1.bam || exit 1 
echo unpaired reads1 mapped at `date`

bwa mem -M -R '@RG\tID:'"$lanename"'\tLB:'"$lanename"'one\tPL:illumina\tSM:TBG_06ta' -t 3 JIC_reference/alygenomes.fasta outtrim/trim_*Arenosa5_R1*_2U.fastq.gz | samtools view -@ 3 -bu - | samtools sort - -@ 3 -m 5G -o sort_Arenosa5_R1_unpaired2.bam || exit 1 
...
mv Arenosa5dedup.bam TBG_06ta_final.bam
mv Arenosa5dedup.bai TBG_06ta_final.bai
